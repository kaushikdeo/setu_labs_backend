/**
 * Seed or update the Recovery Test template for one or all organizations.
 *
 * Usage:
 *   pnpm script:seed-recovery-test --org-id <id> --confirm
 *   pnpm script:seed-recovery-test --all-orgs --confirm
 */
...
import { RECOVERY_TEST_PRESET } from '../src/modules/test-type/presets/recovery-test.preset';
...
async function upsertForOrg(organizationId: string, orgName: string) {
  const existing = await TestTypeModel.findOne({
    ...
    name: RECOVERY_TEST_PRESET.name,
  });

  const payload = {
    ...RECOVERY_TEST_PRESET,
  ...
  if (existing) {
    if (!confirmed) {
      console.log(`  UPDATE ${orgName} — existing "${RECOVERY_TEST_PRESET.name}" (${existing.code})`);
      return 'update';
    }
    ...
    console.log(`  UPDATED ${orgName} — ${RECOVERY_TEST_PRESET.name} (${existing.code})`);
    return 'updated';
  }

  if (!confirmed) {
    console.log(`  CREATE ${orgName} — ${RECOVERY_TEST_PRESET.name}`);
    return 'create';
  }

  const created = await TestTypeModel.create(payload);
  console.log(`  CREATED ${orgName} — ${RECOVERY_TEST_PRESET.name} (${created.code})`);
  return 'created';
}
